...
\begin{definition}
    ...
    \begin{center}
        $\mathrm{H}=\{x:a^{\mathrm{T}}x=b\}$,
    \end{center}
    where $a\in\mathrm{R}^{n}$, $a\neq0$, and $b\in\mathrm{R}$ are given.
\end{definition}

When $b=0$, the hyperplane is simply the set of points that are orthogonal to $a$; when $b\neq0$, the hyperplane is a translation, along direction $a$, of that set.

If $x_{0}\in\mathrm{H}$, then for any other element $x\in\mathrm{H}$, we have
\begin{center}
    $b=a^{\mathrm{T}x_{0}}=a^{\mathrm{T}x}$.
\end{center}

Hence, the hyperplane can be characterized as the set of vectors x such that $x-x_{0}$ is orthogonal to $a$:
\begin{center}
    $\mathrm{H}=\{x:a^{\mathrm{T}}(x-x_{0})=b\}$.
\end{center}
...
\begin{figure}[hbt!]
    \begin{center}   
        \includegraphics[width=120mm]{hyperplane}
