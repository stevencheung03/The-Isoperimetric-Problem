...
\end{note*}
We have just used language such as closed, simple, bounded and compact. We give the definitions of these below.
\begin{definition}
...
\begin{theorem}
Let $C$ be a simple closed curve in the plane with length $L$ and bounding a region of area $A$ . Then $L^2 \leq 4\pi A$ with equality if and only if $C$ is a circle.
\end{theorem}
